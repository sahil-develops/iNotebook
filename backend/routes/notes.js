...
// Route 3 : Add an existing note using : PUT "/api/notes/updatenote/:id". login required
router.put("/updatenote/:id", fetchuser, async (req, res) => {
  ...
  try {
    ...
    if (!note) {
      return res.status(404).send("Kya kar rha ha bhai tu");
    }
    if (note.user.toString() !== req.user.id) {
      return res.status(401).send("Chhod bhai rhne de");
    }
...
// Route 3 : Add an existing note using : DELETE "/api/notes/deletenote/:id". login required
